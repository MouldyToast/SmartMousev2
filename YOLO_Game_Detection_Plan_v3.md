...
## Phase 0: Class Taxonomy (UI-Managed)

Define your classes before capturing or annotating anything. But you never edit config files by hand — the capture tool manages everything.

### Class Manager (built into the capture tool panel)

The capture tool's sidebar has a **Class Manager** section at the top:

1. Type a class name, hit Enter or click **Add**
2. It appears in a numbered list: `0: health_potion`, `1: ore_node`, etc.
3. **Reorder** with Up/Down buttons or drag-and-drop (index = YOLO class ID)
4. **Rename** inline (double-click the name)
5. **Remove** with a delete button (warns if existing annotations reference this class)
6. Every change **auto-writes both files instantly:**
   - `training/classes.txt` — one class name per line (for LabelImg)
   - `training/data.yaml` — full YOLO dataset config with `names:`, `nc:`, `path:`, `train:`, `val:`

**Zero manual file editing.** One UI, zero config files to maintain by hand.

### What to think about when adding classes

For each object your automation needs to detect, answer:
...
3. **Does this object carry readable values the automation needs?**
   ...
   - Mark these in the class manager (checkbox or tag) so the value reading pipeline knows which classes need post-processing

### Example class list (as it would appear in the UI)

```
 #  Class Name          Value Reader
 0  health_potion       —
 1  mana_potion         —
 2  ore_node            —
 3  ore_node_depleted   —
 ...
 6  enemy               —
 7  loot_drop           —
 8  health_bar          bar_fill
 9  mana_bar            bar_fill
10  minimap             —
11  inventory_slot      —
12  action_button       bar_fill (cooldown)
13  tooltip             text_ocr
14  item_stack          digit_ocr
15  damage_number       digit_ocr
```

### Auto-generated `training/data.yaml`

```yaml
# Auto-generated by capture tool — do not edit manually
path: ../datasets/game_objects
train: images/train
val: images/val
nc: 16
names:
...
### Auto-generated `training/classes.txt`
...
- **Adding a class mid-project is fine.** Add it in the class manager, existing annotations are untouched (new class gets the next index). Removing or reordering classes mid-project requires re-indexing existing label files — the class manager handles this automatically and warns before proceeding.
...
labelImg datasets/raw/ training/classes.txt
```
...
- **≥ 0.5:** pre-fill label, human reviews (fast — usually just confirm)
- **0.3 – 0.5:** pre-fill label, flag for careful review (model is guessing)
- **< 0.3:** discard proposal

> **No auto-accept until mAP50 > 0.8.** A model trained on 30 seed images can output 0.85 confidence and still be wrong. High confidence ≠ correctness when the model is young. Keep human review on every proposal for the first 2-3 training cycles. After mAP50 consistently exceeds 0.8, you can introduce auto-accept at ≥0.90 confidence — but only for classes with individual AP > 0.85.
...
```bash
# Small dataset (<100 images) — 1280px default, batch=8 to fit 8GB VRAM
yolo detect train \
  model=yolov8n.pt \
  data=training/data.yaml \
  epochs=100 imgsz=1280 batch=8 device=0 workers=4 \
  freeze=10 patience=30 \
  hsv_h=0.015 hsv_s=0.5 hsv_v=0.3 \
  project=training/runs name=game_v1
...
yolo detect train \
  ...
  patience=40 \
  project=training/runs name=game_v2
  # freeze removed — enough data for full fine-tuning

# If VRAM is tight or UI detection isn't needed
# drop to 640, raise batch size
yolo detect train \
  model=yolov8n.pt \
  data=training/data.yaml \
  epochs=150 imgsz=640 batch=16 device=0 workers=4 \
  patience=40 \
  project=training/runs name=game_v2_lowres
```
...
| `imgsz=1280` | Default for UI-heavy | 640→1280 is a 3× downscale at 1080p. Small UI elements (stack digits, minimap icons) become unreadable at 640. Start at 1280, drop to 640 only if VRAM-constrained or UI classes aren't needed. |
...
Before saving, compute dHash. If hamming distance < 5 from any frame saved in the last 30 seconds, skip. This deduplicates similar views while keeping genuinely different scenes regardless of timing.

> **dHash threshold is < 5 everywhere** (capture dedup, active learning dedup, OCR cache). A single threshold avoids confusion and < 5 is tight enough to reject near-duplicates while accepting meaningful changes like a health bar ticking down.
...
Each active learning cycle adds images and labels. Track what was added when, so you can roll back a bad batch:

```
datasets/game_objects/
├── manifest.jsonl          # append-only log: one line per image added
```

Each line in `manifest.jsonl`:
```json
{"file": "frame_20250214_153022_001.png", "cycle": 3, "source": "active_learn", "added": "2025-02-14T15:30:22"}
```

`dataset_manager.py` writes this automatically on every incorporation. To roll back cycle N: filter manifest, delete matching images/labels, retrain. No git-LFS, no DVC — just a flat log file.
...
After training, export to TensorRT for ~2× faster inference on the 3060 Ti. This is a one-time step after each retrain:

```python
from ultralytics import YOLO
model = YOLO("training/runs/game_v1/weights/best.pt")
model.export(format="engine", half=True)  # creates best.engine
```

The exported `.engine` file loads identically — `YOLO("best.engine")` — so no code changes needed downstream. FP16 halves inference time (2-4ms → 1-2ms) with negligible accuracy loss on detection tasks.

> **Do this after mAP50 > 0.7**, not on every training cycle. Early cycles change models too fast to justify the 2-3 minute export time.
...
model = YOLO("training/runs/game_v1/weights/best.engine")  # TensorRT FP16
...
**Fallback for gradient/segmented bars:** The rightmost-column approach breaks on bars with border decorations, color gradients, or segmented segments. When `filled_cols[-1]` gives an implausible jump between frames (>20% change in a single tick), fall back to **ratio of colored pixels**: `mask.sum() / mask.size`. This is less precise but handles gradients, segments, and decorative borders gracefully. The bar config gains a `method` field: `"edge"` (default, rightmost column) or `"ratio"` (pixel proportion).

**Calibration:** each bar class needs its HSV color range, fill direction, and method defined once. This is a one-time manual step — eyedropper the bar color in an image editor, convert to HSV, set a range. Store in config:
...
bar_configs:
  health_bar:
    ...
    method: edge             # "edge" = rightmost column, "ratio" = pixel proportion
  mana_bar:
...
**Recommended: winocr** — this is a Windows-only project and winocr is faster, requires no binary install, and no PATH configuration. Just `pip install winocr`. Tesseract is the fallback if winocr struggles with a specific game font.
...
for box in results[0].boxes:
    ...
    if reader_type:
        ...
        if value is None:
            continue  # OCR returned garbage — skip, don't propagate None downstream
...
| YOLO inference (v8n, 1280px, TensorRT FP16) | 3-6ms | 3060 Ti, ~2× faster than PyTorch |
...
project/
├── capture/
│   └── capture_tool.py          # Screenshot capture + dedup + hotkeys + class manager UI
├── datasets/
...
│   │   │   ├── train/
│   │   │   └── val/
│   │   └── manifest.jsonl       # Append-only log of additions per cycle
...
│       └── periodic/
├── training/
│   ├── data.yaml                # Auto-generated by class manager
│   ├── classes.txt              # Auto-generated by class manager (for LabelImg)
...
└── tools/
    ├── dataset_manager.py       # Train/val split, incremental additions, manifest
...
> **`training/data.yaml` and `training/classes.txt` are auto-generated.** The capture tool's class manager writes both files on every change. Never edit them by hand.
...
1. Open the capture tool, use the Class Manager to add your initial 8-12 classes (auto-writes `training/classes.txt` + `training/data.yaml`)
2. Tag value-reading classes (bar_fill, digit_ocr, text_ocr) in the class manager
3. Define bar color HSV ranges for any bar-fill classes
...
6. If VRAM-constrained and UI detection isn't critical: try `imgsz=640 batch=16`
