...
# ---------------------------------------------------------------------------
# YOLO auto-prediction integration (Phase 2 patch)
# ---------------------------------------------------------------------------
_YOLO_PREDICTOR = None
try:
    _labelimg_dir = os.path.dirname(os.path.abspath(__file__))
    _parent_dir = os.path.dirname(_labelimg_dir)
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        # --- YOLO auto-prediction setup (Phase 2 patch) ---
        self.yolo_predictor = None
        self._confirmed_count = 0
        self._retrain_every_n = 5

        # Menu toggle for auto-prediction
        self.auto_predict_action = QAction('Auto-Predict (YOLO)', self)
        self.auto_predict_action.setCheckable(True)
        self.auto_predict_action.setChecked(False)
        self.auto_predict_action.setEnabled(_HAS_YOLO)
        self.auto_predict_action.setToolTip(
            'Auto-predict boxes with YOLO when opening unlabeled images')
        self.menus.view.addSeparator()
        self.menus.view.addAction(self.auto_predict_action)

        # Status bar indicator for training
        self.yolo_status_label = QLabel('')
        self.yolo_status_label.setStyleSheet('color: orange; font-weight: bold;')
        self.statusBar().addPermanentWidget(self.yolo_status_label)

        # Timer to poll training status
        self._yolo_status_timer = QTimer(self)
        self._yolo_status_timer.timeout.connect(self._update_yolo_status)
        self._yolo_status_timer.start(2000)

        if not _HAS_YOLO:
            print("  [YOLO] ultralytics not found — auto-predict disabled. "
    ...
    def show_bounding_box_from_annotation_file(self, file_path):
        if self.default_save_dir is not None:
            ...
            # --- YOLO auto-predict: generate .txt if no annotation exists ---
            if (not os.path.isfile(xml_path) and not os.path.isfile(txt_path)
                    and not os.path.isfile(json_path)):
                self._yolo_auto_predict(file_path, txt_path)
    ...
    def _save_file(self, annotation_file_path):
        if annotation_file_path and self.save_labels(annotation_file_path):
            ...
            # --- YOLO: trigger background retrain every N confirmed saves ---
            if self.yolo_predictor is not None:
                self._confirmed_count += 1
                if self._confirmed_count % self._retrain_every_n == 0:
                    data_yaml = getattr(self.yolo_predictor, 'data_yaml', None)
                    if data_yaml:
                        self.yolo_predictor.retrain_background(data_yaml)
                        self.statusBar().showMessage(
                            'Saved. Background retrain started '
                            '(cycle %d, every %d saves)' %
                            (self._confirmed_count // self._retrain_every_n,
                             self._retrain_every_n))
    ...
    # --- YOLO auto-prediction methods (Phase 2 patch) ---

    def init_yolo_predictor(self, weights, data_yaml=None, conf=0.25):
        """Initialize the YOLO predictor. Call after __init__ or from CLI."""
        if not _HAS_YOLO:
            print("  [YOLO] Cannot init — ultralytics not installed.")
            return
        self.yolo_predictor = YOLOPredictor(weights, data_yaml, conf)
        self.auto_predict_action.setChecked(True)
        print(f"  [YOLO] Predictor ready: {weights}")
        if data_yaml:
            print(f"  [YOLO] data.yaml: {data_yaml}")
            print(f"  [YOLO] Retrain every {self._retrain_every_n} saves")

    def _yolo_auto_predict(self, image_path, txt_path):
        """Auto-predict boxes for an unlabeled image and write YOLO .txt."""
        if self.yolo_predictor is None:
            return
        if not self.auto_predict_action.isChecked():
            return
        if self.yolo_predictor.is_training:
            return
        try:
            count = self.yolo_predictor.predict_and_write(image_path, txt_path)
            if count > 0:
                self.statusBar().showMessage(
                    'YOLO predicted %d box%s — review and save (Ctrl+S)' %
                    (count, 'es' if count != 1 else ''))
        except Exception as e:
            print(f"  [YOLO] Auto-predict error: {e}")

    def _update_yolo_status(self):
        """Update the training status indicator in the status bar."""
        if self.yolo_predictor is not None and self.yolo_predictor.is_training:
            self.yolo_status_label.setText(' TRAINING... ')
        else:
...
def get_main_app(argv=None):
    ...
    # --- YOLO integration args (Phase 2 patch) ---
    argparser.add_argument("--yolo-weights", type=str, default=None,
                           help="Path to YOLO weights (.pt) for auto-prediction")
    argparser.add_argument("--yolo-data", type=str, default=None,
                           help="Path to data.yaml for background retraining")
    argparser.add_argument("--yolo-conf", type=float, default=0.25,
                           help="YOLO confidence threshold for auto-prediction")
    argparser.add_argument("--retrain-every", type=int, default=5,
                           help="Retrain after every N confirmed saves (default: 5)")
    ...
    # --- Initialize YOLO predictor if weights provided ---
    if args.yolo_weights and _HAS_YOLO:
        win._retrain_every_n = args.retrain_every
        win.init_yolo_predictor(
            args.yolo_weights,
            args.yolo_data,
            args.yolo_conf)
